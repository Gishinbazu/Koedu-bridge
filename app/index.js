...
export default function HomeScreen() {
  ...
  // Charger l'utilisateur connecté et Rediriger vers l'Admin Dashboard si Admin
  useEffect(() => {
    (async () => {
      try {
        ...
        // ✅ Redirection automatique si le rôle est admin
        const isAdmin =
          userObj?.role === "admin" || userObj?.userRole === "admin";
        if (isAdmin) {
    ...
    <View style={styles.heroContent}>
      <View style={styles.textBlock}>
        ...
        <Text style={[styles.titleLarge, isSmall && mobile.titleLarge]}>
          {isAdmin ? (
            ...
            <>
              ...
              <Text style={{ color: theme.brand }}>with</Text>
              {"\n"}
              <Text style={{ color: theme.brand }}>KOEDU Bridge</Text>
            </>
          )}
        </Text>
        ...
        <Text
          style={[
            styles.description,
            { color: theme.subText, opacity: 0.95 },
            isSmall && mobile.description,
          ]}
        >
          {isAdmin
            ? "Manage your students, applications and KOEDU Bridge content."
            : "Choose your program. Submit your application. Start your journey."}
        </Text>

        {/* ---------- BOUTON PRINCIPAL CENTRAL (Find university and programs) ---------- */}
        {!isAdmin && (
          <View
          ...
          >
            <Pressable
              ...
              style={[
                styles.mainSearchBtn,
                isSmall && mobile.mainSearchBtn,
                { backgroundColor: theme.brand },
              ]}
            >
              <Text style={styles.mainSearchBtnText}>
                Find university and programs
              </Text>
            </Pressable>
          </View>
        )}

        {/* ---------- BOUTONS SECONDAIRES (Apply now & Sign in) ---------- */}
        <View style={[styles.heroButtonsRow, isSmall && mobile.heroButtonsRow]}>
          {isAdmin ? (
            <>
            ...
            <>
              ...
              >
                <Text style={[styles.outlineText, { color: "#ffffff" }]}>
                  Sign in
                </Text>
              </Pressable>
            </>
        ...
        {/* ---------- BOUTON THÈME PILULE (Light Mode / Dark Mode) ---------- */}
        <View style={{ marginTop: 24, alignItems: "center" }}>
          <Pressable onPress={toggleTheme} style={styles.themePillBtn}>
            <Ionicons
...
const styles = StyleSheet.create({
  ...
  // Nouveau bouton principal central
  mainSearchBtn: {
    width: "100%",
    maxWidth: 420,
    paddingVertical: 14,
  ...
  // Alignement des boutons secondaires côte à côte
  heroButtonsRow: {
    flexDirection: "row",
    gap: 12,
    alignItems: "center",
  ...
  // Bouton pour le switch de thème (pilule)
  themePillBtn: {
    flexDirection: "row",
    alignItems: "center",
    paddingVertical: 8,
