...
import styles, { COLORS } from "../../styles/apply/_BaseApplyForm.styles";

// --- STATIC DATA: PRIORITY COUNTRIES ---
const COUNTRIES = [
  "Vietnam",
  "Uzbekistan",
  "China",
...
export default function BaseApplyFormScreen({ programTypeLabel = "Program" }) {
  ...
  const [formData, setFormData] = useState({
    fullName: "",
    nationality: "",
    dob: "", // Stored as DD/MM/YYYY string for display
    phone: "",
    email: "",
  });

  // Pour le web: valeur brute YYYY-MM-DD pour <input type="date" />
  const [dobISO, setDobISO] = useState("");

  // Internal state for Date Picker logic (mobile)
  const [dateObject, setDateObject] = useState(new Date());
  const [showDatePicker, setShowDatePicker] = useState(false);
  const [showCountryPicker, setShowCountryPicker] = useState(false);

  const [eduData, setEduData] = useState({ lastSchool: "", major: "" });
  const [sponsor, setSponsor] = useState("self"); // 'self' or 'parents'
  ...
  const onDateChange = (event, selectedDate) => {
    ...
    if (selectedDate) {
      ...
      handleInput("dob", `${day}/${month}/${year}`);
    }
  };

  // --- Date Picker Handler (WEB) ---
            ...
            <View style={styles.row}>
              ...
              <View style={{ flex: 1 }}>
                {Platform.OS === "web" ? (
                  ...
                  // MOBILE: SelectField + DateTimePicker
                  <SelectField
